...
{
    public partial class EditAlertTemplates : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (((string)Session["LoggedIn"] == "True"))
            {
                ...
                if (IsPostBack == false)
                {
                    try
                    {
                        cmbTemplatesMain.Items.Clear();
                        Myfunc.FillTemplates(cmbTemplatesMain, 0, conStr);
                    }
                    catch
                    {
                    }
        ...
        protected void gvSample_Commands(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (commandName == "EditItem")
            {
                ...
                try
                {
                    DeleteSpecificContactLink(Convert.ToInt32(ViewState["Id"]));
                    LoadContacts();
                }
                catch (Exception ex)
                {
        ...
        protected void gvSample_Commands1(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (commandName == "EditItem")
            {
                ...
                try
                {
                    DeleteSpecificThreshhold(Convert.ToInt32(ViewState["Id"]));
                    LoadThreshholds();
                }
                catch (Exception ex)
                {
        ...
        //    LiveMonitoring.IRemoteLib.UserDetails MyUser = new LiveMonitoring.IRemoteLib.UserDetails();
        //    LiveMonitoring.PageSecurityClass IPMonPageSecure = new LiveMonitoring.PageSecurityClass();
        //    int MyIPMonPageSecure = IPMonPageSecure.GetViewLevelByName(name);
        //    MyUser = (LiveMonitoring.IRemoteLib.UserDetails)Session("UserDetails");
        //    if (MyIPMonPageSecure > MyUser.UserLevel)
        //    {
        //        Response.Redirect("NotAuthorisedView.aspx");
        ...
        //    Collection MyCollection = new Collection();
        //    LiveMonitoring.GlobalRemoteVars MyRem = new LiveMonitoring.GlobalRemoteVars();
        //    MyCollection = MyRem.GetServerObjects(((Session("SelectedSite") == null) ? null : Session("SelectedSite")));
        ...
        //                MyAlert.IncludeImage,
        //                MyAlert.CameraID1,
        //                MyAlert.CameraID2,
        //                MyAlert.SensorValueID1,
        //                MyAlert.SensorValueID2,
        //                MyAlert.SensorValueID3,
        //                MyAlert.SensorValueID4,
        //                MyAlert.Enabled,
        //                MyAlert.SendNormal,
        //                MyAlert.Camera1Delay,
        //                MyAlert.Camera2Delay,
        //                MyAlert.AlertId
        //            }));
        ...
        {
            GridContacts.DataSource = getContacts();
            GridContacts.DataBind();

            GridContactsVerify.DataSource = getContacts();
            GridContactsVerify.DataBind();
        }

        public void LoadThreshholds()
        {
            GridThreashholds.DataSource = getThreshholds();
            GridThreashholds.DataBind();

            GridThreashholdsVerify.DataSource = getThreshholds();
        ...
        private DataTable getContacts()
        {
            ...
            string sqlQuery = "contactstemplate_select_specifictcontacts";
            ...
            da.SelectCommand.Parameters.AddWithValue("TemplateId", cmbTemplatesMain.SelectedValue);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        private DataTable getThreshholds()
        {
            LiveMonitoring.IRemoteLib.UserDetails MyUser = new LiveMonitoring.IRemoteLib.UserDetails();
            MyUser = (LiveMonitoring.IRemoteLib.UserDetails)Session["UserDetails"];

            string sqlQuery = "alerts_threshholdstemplate_select_specific";
            SqlConnection connection = new SqlConnection(conStr);
            if (connection.State == ConnectionState.Closed)
            {
                connection.Open();
        ...
        public bool SaveContactLink()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "alertcontactTemplatelink_add_new";
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection = con;

                cmd.Parameters.Clear();

                cmd.Parameters.Add(paramTemplateId).Value = cmbTemplatesMain.SelectedValue;
        ...
        public bool SaveThreshholdTemp(LiveMonitoring.IRemoteLib.AlertDetails.AlertThreshholdsDef Threshhold)
        {
            ...
            if ((MySQLReader == null) == false)
            {
                while (MySQLReader.Read())
                {
                    ...
                    {
                        MyNewID = (int)MySQLReader.GetValue(0);
                        if (MyNewID > 0)
                        {
                            Saved = true;
                        }
        ...
        {
            if (cmbContacts.SelectedIndex != -1 & cmbTemplatesMain.SelectedItem != null)
            {
                try
                {
                    if ((SaveContactLink() == true))
                    {
                        LoadContacts();
            ...
            else
            {
                ...
                {
                    errorMessage.Visible = true;
                    lblError.Text = "Error Please select contact and template !";
                }
                else
                {
                    errorMessage.Visible = true;
        ...
        protected void cmdSend_Click_SaveThreashold(object sender, EventArgs e)
        {
            ...
            MyAlerts.Name = this.txtName.Text;
            MyAlerts.SensorID = Convert.ToInt32(this.cmbSensorID.SelectedValue);
            MyAlerts.DeviceID = Convert.ToInt32(this.cmbDeviceID.SelectedValue);
            MyAlerts.TestType = (LiveMonitoring.IRemoteLib.TestType)Convert.ToInt32(this.TestType.SelectedValue);
            MyAlerts.Field = Convert.ToInt32(this.cmbField.SelectedValue);
            MyAlerts.CheckValue = Convert.ToInt32(this.txtCheckValue.Text);
            MyAlerts.HoldPeriod = Convert.ToInt32(this.txtHoldPeriod.Text);
            //???
            MyAlerts.AlertID = Convert.ToInt32(cmbTemplatesMain.SelectedValue);
            MyAlerts.Comparison = this.Comparison.Items[0].Selected;
            MyAlerts.Order = Convert.ToInt32(this.txtOrder.Text);
            MyAlerts.Extra = this.TxtExtra.Text;
            MyAlerts.Extra1 = this.TxtExtra1.Text;
            if (!string.IsNullOrEmpty(this.TxtExtra2.Text))
            {
                MyAlerts.Extra2 = Convert.ToDouble(this.TxtExtra2.Text);
                MyAlerts.Extra3 = Convert.ToDouble(this.TxtExtra3.Text);
            }
            MyAlerts.TabularCnt = Convert.ToInt32(this.txtTabularCnt.Text);
            if (cmbFieldComp.Visible == true)
            {
                MyAlerts.Field1 = Convert.ToInt32(cmbFieldComp.SelectedValue);
            }
            else
            {
                MyAlerts.Field1 = 0;
            }
            MyAlerts.IsTemplate = this.chkSensAlertTemplate.Checked;
            LiveMonitoring.GlobalRemoteVars MyRem = new LiveMonitoring.GlobalRemoteVars();
        ...
        protected void Wizard1_FinishButtonClick(object sender, WizardNavigationEventArgs e)
        {
            ...
            try
            {
                ...
                if (MyAlert.AlertType == 0)
                {
                    lblMessageSucess.Visible = true;
                    lblMessageSucess.Text = "Message Type:Please select Alert Type!";
                    return;
                }
                if (string.IsNullOrEmpty(MyAlert.AlertMessage))
                {
                    lblMessageSucess.Visible = true;
                    lblMessageSucess.Text = "Message:Please Enter a Message!";
                    return;
                }
                int aCnt = 0;
                ...
                MyAlert.AlertId = Convert.ToInt32(cmbTemplatesMain.SelectedValue);
                LiveMonitoring.GlobalRemoteVars MyRem = new LiveMonitoring.GlobalRemoteVars();
                int returns = MyRem.LiveMonServer.EditAlertTemplate(MyAlert);

                lblMyID.Text = cmbTemplatesMain.SelectedValue;

                if (returns < 0)
                {
                    lblMessageSucess.Visible = true;
                    lblMessageSucess.Text = "Message, Message Type & Options:Not saved Error!";
                    try
                    {
                ...
                else
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            Response.Redirect("EditAlertTemplates.aspx");
                        }
                        catch
                        {
                        }
        ...
        protected void GridContacts_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DeleteSpecificContactLink(Convert.ToInt32(GridContacts.SelectedRow.Cells[9].Text));
                LoadContacts();
            }
            catch (Exception ex)
            {
        ...
        protected void GridThreashholds_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DeleteSpecificThreshhold(Convert.ToInt32(GridThreashholds.SelectedRow.Cells[1].Text));
                LoadThreshholds();
        ...
        protected void cmbTemplatesMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            LoadTemplateDetails();
        }
        ...
        public void LoadTemplateDetails()
        {
            try
            {
                ...
                AlertMessage.Text = "";
                DataRow dr = (new LiveMonitoring.SharedFuncs()).GetTemplate(Convert.ToInt32(cmbTemplatesMain.SelectedValue), conStr);
                txtTemplateName.Text = (string)dr["Template"];
                AlertMessage.Text = (string)dr["Message"];

                for (int j = 0; j <= this.AlertType.Items.Count - 1; j++)
                {
                    if (AlertType.Items[j].Value == (string)dr["Type"])
                    {
                        AlertType.Items[j].Selected = true;
                    }
                    else
                    {
                        AlertType.Items[j].Selected = false;
                    }
                }

                if ((bool)dr["IncludeImage"] == true)
                {
                    this.AlertIncludeImage.Items[0].Selected = true;
                }
                else
                {
                    this.AlertIncludeImage.Items[1].Selected = true;

                }
                txtDelay1.Text = (string)dr["Camera1Delay"];
                txtDelay2.Text = (string)dr["Camera2Delay"];


                if ((bool)dr["Enabled"] == true)
                {
