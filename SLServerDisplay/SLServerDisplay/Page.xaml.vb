...
Partial Public Class Page
    ...
    Dim displaysDropdown As Windows.Controls.ComboBox
    ...
    Private TriedPassedLogin As Boolean = False
    ...
    Public displayIdNameList As IDictionary(Of String, Integer)
    ...
    Public UserLoggedIn As UserDetails
    'Public IsEditInEffect As Boolean = False
    ...
    Private Sub SetParameters(ByVal ParameterID As String, ByVal Value As String)
        If _parameters.ContainsKey(ParameterID) Then
    ...
    Private Sub MyButtClicked(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs)
        ...
        For Each Mypage As ServerDisplay In MyPages
            Mypage.isInEditInEffect = False
        Next
    End Sub
    ...
    Private Sub displaysDropdownChanged(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs)

        If selectedDisplayName <> displaysDropdown.SelectedItem Then

            selectedDisplayName = displaysDropdown.SelectedItem
            Dim selectedID As Integer = displayIdNameList.Item(selectedDisplayName)

            Dim slServerDisplayURI As New Uri(HtmlPage.Document.DocumentUri, "SLServerDisplay.aspx?DisplayID=" + selectedID.ToString)
            HtmlPage.Window.Navigate(slServerDisplayURI)

            SetParameters("DisplayID", selectedID)
    ...
    Public Sub New(ByVal parameters As IDictionary(Of String, String))
        ...
        Try
            ...
            displaysDropdown = New Windows.Controls.ComboBox
            displaysDropdown.Width = 140

            imageroot.Children.Add(displaysDropdown)
            displaysDropdown.Visibility = Windows.Visibility.Collapsed
            AddHandler displaysDropdown.SelectionChanged, AddressOf displaysDropdownChanged
    ...
    Private Sub Sitesproxy_GetDisplaysCompleted(sender As Object, e As GetDisplaysCompletedEventArgs) Handles Sitesproxy.GetDisplaysCompleted
        ...
        displayIdNameList = New Dictionary(Of String, Integer)
        Dim selectedIndex As Integer

        Try

            If IsNothing(e.Result) = False Then
                displaysDropdown.Items.Clear()
                If e.Result.Count() > 0 Then

                    For displaysCounter As Integer = 0 To e.Result.Count - 1
                        Try
                            Dim MyString() = e.Result.Item(displaysCounter).Split("|")
                            Dim displayListitem As New ListBoxItem
                            Dim currentlyDisplayedListitem As New ListBoxItem
                            Dim currentDisplayName As String = String.Empty

                            displayListitem.BorderBrush = New System.Windows.Media.SolidColorBrush(Colors.Black)
                            displayListitem.BorderThickness = New Thickness(2, 2, 2, 2)

                            'displayListitem.Content += "Name:" + MyString(0)
                            'displayListitem.Content += "  Type:" + MyString(3)
                            'displayListitem.Content += "  ID:" + MyString(1)
                            displayListitem.Tag = e.Result(displaysCounter)

                            If MyString(1) = CInt(Me.GetParameters("DisplayID")) Then
                                currentDisplayName = MyString(0)
                                selectedDisplayName = MyString(0)
                                currentlyDisplayedListitem = displayListitem

                            End If

                            'displaysDropdown.Items.Add(displayListitem)
                            displaysDropdown.Items.Add(MyString(0))
                            displayIdNameList.Add(MyString(0), MyString(1))

                            selectedIndex = displaysDropdown.Items.IndexOf(currentlyDisplayedListitem)

                            displaysDropdown.SelectedItem = currentDisplayName
    ...
    Private Sub Sitesproxy_CheckLoginCompleted(sender As Object, e As CheckLoginCompletedEventArgs) Handles Sitesproxy.CheckLoginCompleted
        Try
            If e.Result <> "" Then
                ...
                'Me.Close()
                LoginSucceed(MyUserDet)
            Else 'not auto try show login
                UserLogin()
    ...
    Private Sub UserLogin()
        If IsNothing(UserLoggedIn) = True Then
            If TriedPassedLogin = False Then
                TriedPassedLogin = True
                Try
                    Try
                        Dim pstrPassword As String = Me.GetParameters("Password")
                        Dim pstrUserName As String = Me.GetParameters("UserName")
                ...
                End Try
            Else
                Dim MyLoginPage As ChildLogon = New ChildLogon(_parameters)
                MyLoginPage.Show()
                AddHandler MyLoginPage.LoginSucceeded, AddressOf LoginSucceed
            End If           
        Else
            LoginSucceed(UserLoggedIn)
        End If
    End Sub
    Private Sub LoginSucceed(ByVal User As UserDetails)
        UserLoggedIn = User
        Dim Mybutt As Windows.Controls.Button = imageroot.FindName("txtMyButt")
        Dim Mybutt1 As Windows.Controls.Button = imageroot.FindName("txtMyButtCancel")
        'txtMyMenuButt
        ...
        displaysDropdown.Visibility = Windows.Visibility.Visible
        For Each Mypage As ServerDisplay In MyPages
            Mypage.isInEditInEffect = True
        Next
    End Sub
